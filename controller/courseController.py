...
async def get_course(page: int = 1, page_size: int = 10, db: AsyncSession = Depends(get_db)):
...
async def get_course_by_conditions(query_params: CourseQueryRequest, db: AsyncSession = Depends(get_db)):
...
async def get_course_detail(course_code: str, db: AsyncSession = Depends(get_db)):
    try:
...
async def add_course(course_data: CourseRequest, db: AsyncSession = Depends(get_db)):
    try:
...
async def update_course(course_data: CourseUpdateRequest, db: AsyncSession = Depends(get_db)):
    try:
